...
(function (ns) {
  ...
  function appHtml(model) {
    ...
    return '<!doctype html><html><head><base target="_blank"><meta name="viewport" content="width=device-width,initial-scale=1">' +
      ...
      '#toast{position:sticky;bottom:0;margin-top:12px}.empty{text-align:center;padding:20px 10px;color:#5f6368}.boot-state{padding:24px 4px;color:#5f6368;font-size:12px}' +
      '</style></head><body><div id="root"><div class="boot-state">Loading Spoti Sync…</div></div><div id="toast"></div>' +
      '<script>' +
      ...
      'function toast(text,type){var t=el("toast");if(!t)return;t.innerHTML=text?("<div class=\\"banner "+(type||"")+"\\">"+esc(text)+"</div>"):"";}function showBootError(detail){var r=el("root");if(!r)return;var message=detail?String(detail).slice(0,240):"Unexpected error.";r.innerHTML="<div class=\\"banner bad\\"><strong>Spoti Sync could not start.</strong><br>Reload the Sheet and try again.<div class=\\"small\\">"+esc(message)+"</div></div>";}window.addEventListener("error",function(e){showBootError(e&&e.message?e.message:"Unexpected error.");});window.addEventListener("unhandledrejection",function(e){var reason=e&&e.reason;showBootError(reason&&reason.message?reason.message:reason||"Unexpected error.");});' +
      ...
      'function renderHeader(){return "<div class=\\"top\\"><div><h2>Spoti Sync</h2><div class=\\"muted\\">Spotify automation, kept simple.</div></div><div class=\\"version\\">v"+esc(STATE.version)+"</div></div>";}' +
      'function renderHome(model){STATE=model||STATE;EDITOR=null;SOURCE_SELECTED="";TARGET_SELECTED="";var r=el("root");var html=renderHeader();' +
      ...
      'function renderAutomation(){return "<div class=\\"section\\"><label>Automation</label>"+(EDITOR.automationOptions||[]).map(function(o){var radio="<input type=\\"radio\\" name=\\"automation\\" value=\\""+esc(o.value)+"\\"> ";if(o.intervalUnit==="hours"){return "<label class=\\"choice\\">"+radio+"Every <input id=\\"intervalHours\\" type=\\"number\\" min=\\""+EDITOR.frequencyLimits.hours.min+"\\" max=\\""+EDITOR.frequencyLimits.hours.max+"\\" step=\\"1\\" style=\\"width:72px\\"> hours</label>";}if(o.intervalUnit==="days"){return "<label class=\\"choice\\">"+radio+"Every <input id=\\"intervalDays\\" type=\\"number\\" min=\\""+EDITOR.frequencyLimits.days.min+"\\" max=\\""+EDITOR.frequencyLimits.days.max+"\\" step=\\"1\\" style=\\"width:72px\\"> days</label>";}return "<label class=\\"choice\\">"+radio+esc(o.label)+"</label>";}).join("")+"</div>";}' +
      'function renderEditor(){var c=EDITOR.config;var html=renderHeader()+"<button class=\\"secondary\\" id=\\"back\\">← Back</button><h3>"+(EDITOR.mode==="edit"?"Edit job":"Add job")+"</h3>";if(EDITOR.catalogWarning){html+="<div class=\\"banner warn\\">"+esc(EDITOR.catalogWarning)+"</div>";}' +
      ...
      'html+="<div class=\\"section\\"><label>Behavior</label><select id=\\"behavior\\"></select></div>"+renderAutomation();' +
