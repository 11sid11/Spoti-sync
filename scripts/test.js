...
function snapshot(ids, ordering = Constants.ORDERING.PRESERVE, ignoredCount = 0) {
  return { tracks: ids.map(track), ordering, ignoredCount };
}
...
function jobRow(overrides = {}) {
  return [
    ...
    overrides.lastError || '',
    overrides.heartbeatEnabled !== undefined ? overrides.heartbeatEnabled : true
  ];
}
...
(function testFrontInsertionBatchesPreserveOrder() {
  ...
  let playlist = [];
  batches.forEach((batch) => { playlist = batch.concat(playlist); });
...
(function testAppendNeverRemovesAndKeepsChronologicalLikedOrder() {
  ...
  assert.deepStrictEqual(Array.from(plan.add, (item) => item.keyUri), [
    'spotify:track:MIDDLE', 'spotify:track:NEWEST'
  ]);
  assert.strictEqual(plan.removeCount, 0);
  assert.strictEqual(plan.addMode, 'END');
})();

(function testJobModelParsesAutomationAndHeartbeatWithoutPresentationDependency() {
  const parsed = SheetStore._parseJobRows([
    jobRow({ behavior: 'Append Only', frequency: 'Every 10 days', heartbeatEnabled: false })
  ], 2);
...
(function testAutomationOffJobStillRetainsRunnableConfiguration() {
  const parsed = SheetStore._parseJobRows([
    jobRow({ enabled: false, behavior: 'Append Only', frequency: 'Every 21 days' })
  ], 2);
  assert.strictEqual(parsed.errors.length, 0);
  assert.strictEqual(parsed.jobs.length, 1);
...
(function testMalformedAutomatedJobDoesNotBlockValidJobs() {
  const parsed = SheetStore._parseJobRows([
...
(function testLegacyMigrationPreservesIdsTelemetryAndDefaultsHeartbeatOn() {
  const migrated = SheetStore._legacyJobToStoredRow([
    true, 'Shareable Likes', 'LIKED_SONGS', '',
...
(function testSemanticVersionComparison() {
  assert(UpdateChecker.compareVersions('1.4.0', '1.3.8') > 0);
  assert(UpdateChecker.compareVersions('1.4.0', '1.4.0') === 0);
  assert(UpdateChecker.compareVersions('1.4.0', '1.4.0-beta.9') > 0);
...
(function testPublishedVersionMetadataMatchesSource() {
  ...
  assert.strictEqual(Constants.SHEETS.SUMMARY, 'Spoti Sync');
})();
...
(function testBundleSafetyAndEndpointGuards() {
  ...
  assert(bundle.includes('runJob'));
  assert(bundle.includes('Scheduler.reconcile'));
...
(function testOneCanonicalInstallerSourceManifest() {
  ...
  assert(sourceManifest.includes('92_JobEditor.gs'));
  assert.deepStrictEqual([...sourceManifest].sort(), sourceModules);
  assert.strictEqual(new Set(sourceManifest).size, sourceManifest.length);
  ...
  assert(!buildScript.includes('const sourceFiles = ['));
  assert(!siteScript.includes('const sourceFiles = ['));
})();

(function testPagesInstallerDescribesSingleSurfaceProduct() {
...
console.log('All Spoti Sync core and v1.4 ownership tests passed.');
